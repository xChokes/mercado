...
class BancoCentral:
    ...
    def ejecutar_politica_monetaria(self, ciclo):
        ...
        return {
            ...
            # Formato esperado por main.py
            'accion_tomada': True,
            'descripcion': f"Política {decision['accion']}",
            'nueva_tasa': self.tasa_interes_base * 100  # Convertir a porcentaje
        }
    ...
    def calcular_inflacion(self):
        ...
        if not hasattr(self.mercado, 'indices_precios_historicos') or len(self.mercado.indices_precios_historicos) < 2:
            return 0.0
        
        indice_actual = self.mercado.indices_precios_historicos[-1]
        indice_anterior = self.mercado.indices_precios_historicos[-2]
        
        if indice_anterior > 0:
            return (indice_actual - indice_anterior) / indice_anterior
        return 0.0
