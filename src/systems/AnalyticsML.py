...
class PredictorDemanda:
    ...
    def extraer_caracteristicas(self, mercado, bien):
        ...
        fases = {'expansion': 1, 'recesion': 0.3, 'depresion': 0, 'recuperacion': 0.7}
    ...
    def _prediccion_basica(self, mercado, bien):
        ...
        if transacciones_bien:
            demanda_promedio = sum([t.get('cantidad', 1) for t in transacciones_bien]) / len(transacciones_bien)
...
class ClusterizadorAgentes:
    ...
    def extraer_caracteristicas_consumidor(self, consumidor):
        ...
        total_bienes = sum([len(lista_bienes) for lista_bienes in consumidor.bienes.values()])
    ...
    def clusterizar_consumidores(self, mercado):
        ...
        for consumidor in consumidores:
            caracteristicas = self.extraer_caracteristicas_consumidor(consumidor)
            X.append(caracteristicas)
    ...
    def analizar_clusters(self, clusters):
        ...
        for cluster_id, consumidores in clusters.items():
            ...
            dinero_promedio = sum([c.dinero for c in consumidores]) / len(consumidores)
            ingreso_promedio = sum([c.ingreso_mensual for c in consumidores]) / len(consumidores)
            empleados = sum([1 for c in consumidores if c.empleado])
            tasa_empleo = empleados / len(consumidores)
            
            propension_promedio = sum([c.propension_consumo for c in consumidores]) / len(consumidores)
            aversion_promedio = sum([c.aversion_riesgo for c in consumidores]) / len(consumidores)
